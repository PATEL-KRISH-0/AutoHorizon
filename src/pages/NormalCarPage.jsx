...
export default function NormalCarPage() {
  // States for cars, loading status, and error detection
  ...
  // Navigation function for car detail page
  const handleCarClick = (id) => {
    navigate(`/carOverview?id=${id}`);
  };

  // Mapping objects to convert filter values to expected casing
  const fuelTypeMap = {
    electric: "Electric",
    hybrid: "Hybrid",
    petrol: "Gasoline",
  ...
  // Build query string from filter states and fetch cars
  const applyFilters = () => {
  ...
  // Fetch cars with filters. If a 404 status is received, treat it as "no data found".
  const fetchCars = async (query = "") => {
    ...
    try {
      ...
      if (response.status === 404) {
        // 404 indicates no cars found for the query; set noDataFound and clear previous list.
        setNoDataFound(true);
        setCars([]);
        return;
      }
      ...
      if (!data.data || data.data.results === 0) {
        ...
        // Only include normal cars (exclude auctions)
        const normalCars = data.data.documents.filter((car) => !car.isAuction);
        setCars(normalCars);
      }
    } catch (err) {
      console.error(err);
        ...
        <section className={styles["filters"]}>
          ...
          <div className={styles["filter-group"]}>
            ...
            <div className={styles["brand-grid"]}>
              {["tesla", "porsche", "audi", "bmw", "mercedes", "mitsubishi"].map(
                (brand) => (
                  <div
                    key={brand}
